...
# torch.cuda.set_device(range(num_gpu))
if num_gpu == 1:
...
#     n.append(i)
# h = str(",".join(str(x) for x in n))
# os.environ["CUDA_VISIBLE_DEVICES"] = h
...
def load_raw_datasets(cfg: DictConfig) -> DatasetDict:
    ...
    # sampler = torch.utils.data.distributed.DistributedSampler(raw_datasets, shuffle=True, num_replicas = num_gpu, seed = 31)
    return raw_datasets
...
def main(cfg: DictConfig):
    ...
    # tokenizer, model = tokenizer.to(device), model.to(device)
    ...
    train_dataset = tokenized_datasets["train"] #cuda(device)
    eval_dataset = tokenized_datasets["validation"] #.to(device)
